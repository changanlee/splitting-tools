...
describe("settle — conservation invariant (FR50 CI gate)", () => {
    ...
    const lines: SettleLine[] = [
      {
        ...
        shareCount: 1, isIrc: false,
        claimable: true,
        orphan: false,
      },
      { id: "L2", netCents: 9990, shareCount: 1, isIrc: false, claimable: true, orphan: false },
      // Folded (non-orphan) IRC — already in L1's net; settlement skips it
      {
        id: "L3",
        netCents: -900,
        shareCount: 1, isIrc: true,
        claimable: false,
        orphan: false,
      },
    ];
  ...
  it("unclaimed lines flow to pendingCents (not lost)", () => {
    const lines: SettleLine[] = [
      { id: "L1", netCents: 1000, shareCount: 1, isIrc: false, claimable: true, orphan: false },
      { id: "L2", netCents: 500, shareCount: 1, isIrc: false, claimable: true, orphan: false },
    ];
    const claims: SettleClaim[] = [
      { receiptLineId: "L1", identityId: "A", weight: 1 },
    ];
  ...
  it("orphan IRC flows to orphanIrcCents (kept, negative)", () => {
    const lines: SettleLine[] = [
      { id: "L1", netCents: 1000, shareCount: 1, isIrc: false, claimable: true, orphan: false },
      { id: "I1", netCents: -200, shareCount: 1, isIrc: true, claimable: false, orphan: true },
    ];
    const claims: SettleClaim[] = [
      { receiptLineId: "L1", identityId: "A", weight: 1 },
    ];
...
describe("settle — #5564 placeholder scale (FR50 anchor)", () => {
  it("placeholder 28-line synthetic #5564 nets to 220850 (matches 1.5 algorithm contract)", () => {
    ...
    const ircRows: SettleLine[] = [
      { id: "i0", netCents: -4000, shareCount: 1, isIrc: true, claimable: false, orphan: false },
      { id: "i1", netCents: -3500, shareCount: 1, isIrc: true, claimable: false, orphan: false },
      { id: "i2", netCents: -2500, shareCount: 1, isIrc: true, claimable: false, orphan: false },
    ];
