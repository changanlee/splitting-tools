...
const SYSTEM_INSTRUCTION =
  ...
  "(2) lines＝逐行抽出每個品項：" +
  "description＝一律輸出純【繁體中文】品名（給使用者分帳看）。" +
  "若品項是外語（韓／日／英／泰等），整筆翻成道地繁中——【品牌也要翻】成通用中文譯名（例：농심→農心、오뚜기→不倒翁、크라운→皇冠、종가→宗家、덴마크→丹麥、남양→南陽、오리온→好麗友、해태→海太），沒有通用中文譯名才用英文（例：Oreo、Pocky、Binch、Activia）。" +
  "【description 絕對不可包含任何韓文諺文或日文假名等外文原字元】；無法確定正確譯名時，仍要給出最合理的繁中翻譯，並把該行標成 descriptionConfidence=\"low\"（不可因此保留外文）。把天書縮寫也還原成可辨識中文品名。" +
  "rawText＝一律保留該行【原始文字】（含外語原文與縮寫，逐字照抄），供後續對帳追溯。" +
  "descriptionConfidence＝你對「description 是否為正確／通用譯名」的把握：音譯猜測、罕見品牌、無法確定官方譯名→\"low\"；常見品項、本來就是中文收據→\"high\"。寧可標 low 也不要亂猜當 high。" +
  "qty＝數量（正整數）；" +
  "amountCents＝該行金額換算成【整數最小貨幣單位】，依該收據幣別的小數位數：" +
  "有 2 位小數的幣別（USD／TWD／CNY／HKD／EUR／GBP／SGD／THB／MYR…）→ 金額×100（16.50→1650）；" +
  "【0 位小數的幣別（KRW 韓元／JPY 日圓／VND…）→ 直接用整數金額、不要×100】（₩4,980→4980、¥1,200→1200）；嚴禁小數。" +
