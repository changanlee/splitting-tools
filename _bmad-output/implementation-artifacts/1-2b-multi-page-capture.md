...
5. **AC5（每頁強制決斷，decided-by-construction）** Given 任一頁，Then 該頁**只有在** per-page 決斷已滿足（`hasUsableMaskOrSkip(rects, skip) === true`，沿用 1.2 之 `applyMaskAndEncode` confirm 路徑）後，**才**被加入頁面清單——清單中不存在「未決斷頁」。**「重編」之實現（by-design，NFR-S3 衍生）**：遮罩為不可逆像素燒入，已遮頁無法 in-place 重遮；故「重編某頁」＝**移除＋再拍下一段**（非回到 editing 重遮）。此設計比字面更強——清單中**結構上**不可能存在未決斷頁。PO 若要求字面「回退重編」可提出，但須先放寬 NFR-S3（不建議）。`allPagesDecided` 為**防衛性不變量**（清單非空且每頁 `decided===true`）；「完成」鍵 gating ＝ `allPagesDecided(list)`（在 decided-by-construction 下即等同「清單非空」，但仍以此純函式為唯一判準，杜絕繞過）。
...
- [x] **Task 5：頁面清單 UI（review 階段，client component）（AC3, AC8）**
  - [x] 新增 `src/features/parsing/components/PageList.tsx`：每頁已遮縮圖（`size-16`、`object-contain` 不裁切遮罩區）+ 上移/下移/移除**控制鍵 `size-12`=48px**（`aria-label`、Tab/Enter、文字「第 N 頁」+icon+語意色三重編碼）；CaptureFlow review 區「再拍下一段」「完成（allPagesDecided 才 enable）」「全部重拍」；不做拖曳排序/自動拼接。
...
## Review Findings（code review 2026-05-19，**full 模式**，commit 0b176c8）

> 3 層平行（Blind / Edge Case / Acceptance Auditor；非 LLM-boundary 故 LLM
> Compliance 自動跳過）。full 模式 Acceptance Auditor 有跑（依「有 spec 用
> full」教訓）。所有 patch 已自主修復並通過閘門，1-2b → done。

- [x] [Review][Patch][High] `onFileChange`/`finish` 無 phase 守衛 → 「再拍下一段」開檔中途按「完成」會被晚到的檔覆蓋丟掉已完成 `Blob[]`；finish 可重入。修：onFileChange 僅在 phase∈{idle,review,error} 受理；finish 非 review 即 no-op。
- [x] [Review][Patch][High] `computeSignature` 只取前 1KB+size → 同機不同頁共用 JPEG 表頭 → 簽章碰撞 → `dedupePages` 靜默丟真實頁（資料完整性）。修：改 hash **整顆已遮 blob**（`blob.arrayBuffer()`）；pages.ts 文件同步更新。
- [x] [Review][Patch][Low] review 移除最後一頁 → 空清單死狀態。修：`handleRemove` 清空即回 idle。
- [x] [Review][Patch][Low] `resetAll` 用滯後 `pagesRef` → 潛在漏/重複 revoke。修：改 functional `setPages` updater 從真實 prev revoke。
- [x] [Review][Patch][Low] 縮圖 `object-cover` 裁掉遮罩區（驗證用途反效果）。修：`object-contain`+`bg-muted`。
- [x] [Review][Patch][Doc] pages.ts 標註 `nextPageId` 為唯一刻意非純；Task5 縮圖實為 size-16（控制鍵 size-12=48px）已更正。
- [x] [Review][By-design] AC5「重編」字面未實作 → 經 NFR-S3 論證以 remove+recapture 滿足（更強：結構上無未決斷頁），已 codify 入 AC5 文字（非 silent，PO 可異議）。
- [Dismiss×~7]：error 取消 picker（1.2 parity，重試可恢復）、size>4GiB wrap（收據不可達）、totalKb NaN（不可達）、aria index 暫時不一致（re-render 原子一致）、generic 不對稱（無 bug）、onRetake 流程（非 bug）。
- 驗證：typecheck/lint/test（3 files 44 pass+2 todo，0 回歸）/build 全綠；AC9 靜態掃描——createObjectURL 僅 1 處（masked blob），revoke 4 路徑配對，未遮零洩漏。
- Change Log：2026-05-19 code review（full）→ 5 patch 修復 + 1 by-design codify，閘門綠，Status review→done。
