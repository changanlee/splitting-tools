...
- ~~FR7: 系統 can 拒絕非 #5564 同結構的收據並明確告知不支援（v1 硬鎖）~~ **REVERTED 2026-05-20**：scope 化簡為「OCR 任何 LLM 解析得出的收據」（user direct request, pre-launch 0 user）。Story 1.6 程式碼下架（`structureGuard.ts` 刪除、parseWorker FR7 閘門移除、`STRUCTURE_REJECT_MESSAGE` 移除）；Story 1.6 spec 檔保留為歷史記錄但 status 由 done→reverted。
